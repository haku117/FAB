...
for m in $MODE; do for i in 2 4 8 16; do for BS in 1000 5000 10000 20000;
do echo $i-$m-$BS -- $(date);
set_dir
mpirun -n $((i+1)) -hostfile myhosts src/main/main $m $ALG $PARAM $DATADIR/$ALG-$PARAM-$DSIZE-d.csv $RESDIR/$m-$i.csv -1 $YLIST 0 $LR $BS $ITER $TIME 1000 0.5 200 --v=4 > $LOGDIR/$m-$i;
#done
